on adding folder items to thisFolder after receiving addedItems
	repeat with addedItem in addedItems
		tell application "Finder"
			set itemName to (name of addedItem)
			my packMaster(POSIX path of addedItem)
			my sendToLaura(itemName)
		end tell
	end repeat
...
on packMaster(folderPath)
	tell application "System Events"
		if class of (disk item folderPath) is folder then
			...
			set targetFolder to "/Users/HD6904/Desktop/Master Library/" & thisName
			do shell script ("mkdir -p \"" & targetFolder & "\"")
			set copyList to {}
			set end of copyList to (POSIX path of (folder "Orders" of projectFolder))
			set end of copyList to (POSIX path of (folder "Outline" of projectFolder))
			set smID to (find text "^\\d{4}( V\\d| R\\d)?( V\\d| R\\d)?" in thisName with regexp and string result)
			set end of copyList to (POSIX path of (item 1 of (get files of folder ":Users:HD6904:Public:Eriks Dropbox:-Recent Manuals:" whose name begins with smID)))
