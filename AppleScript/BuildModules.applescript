...
function processTOC (toc, tocNum) {
	...
	var drawText = bash('/usr/local/bin/pdftotext -layout -nopgbrk ' + quote(toc) + ' -'),
		drawMatches = drawText.match(/^ {15,30}([-0-9]{8,}|(\d-)?([A-Z]{2}[- ]?)\d{5})\S*/mg),
...
function checkPartials (drawNum) {
	var baseNum = drawNum.replace(/-\d\d\d?$/, ''),
		fname = baseNum + '*.-.pdf',
		findcmd = 'find ' + dirs.map(d => quote(d)).join(' ') + ' -iname \'' + fname + '\'',
		found = bash(findcmd).split(/\r|\n/);
	if (found.length) return found[0];
	return undefined;
}