...
while read -rd '' filepath; do
	...
	if [[ "$drawrev" == '-' ]]; then
		newspecrev=$(curl --anyauth -u $login "http://houston/ErpWeb/PartDetails.aspx?PartNumber=$partnum" | grep revisionNum | sed -E 's/^.+>([A-Z0-9]+)<.+$/\1/p')
		if val $specrev $newspecrev; then
			rm "$filepath"
			curl -o "./HOPPER/$partnum.pdf" "http://houston/ErpWeb/Part/PartDocumentReader.aspx?PartNumber=${partnum}&checkInProcess=1" -H 'Cookie: DqUserInfo=PartDocumentReader=AMERICAS\MorganEL' -H 'Connection: keep-alive' --compressed
			mv "./HOPPER/$partnum.pdf" "$dirpath/$partnum.$drawrev.$newspecrev.pdf"
		fi
	elif [[ "$specrev" == '-' ]]; then
		newdrawrev=$(curl --anyauth -u $login "http://houston/ErpWeb/PartDetails.aspx?PartNumber=$partnum" | grep revisionNum | sed -E 's/^.+>([A-Z0-9]+)<.+$/\1/p')
		if val $drawrev $newdrawrev; then
			pullDraw+=("$partnum")
		fi
	else
		drawnum="${partnum%-*}"
		newdrawrev=$(curl --anyauth -u $login "http://houston/ErpWeb/PartDetails.aspx?PartNumber=$drawnum" | grep revisionNum | sed -E 's/^.+>([A-Z0-9]+)<.+$/\1/p')
		newspecrev=$(curl --anyauth -u $login "http://houston/ErpWeb/PartDetails.aspx?PartNumber=$partnum" | grep revisionNum | sed -E 's/^.+>([A-Z0-9]+)<.+$/\1/p')
		if val $drawrev $newdrawrev; then
			pullDraw+=("$drawnum")
		fi
...
uniq pull-list.txt pull-list.txt
rm ./HOPPER/*.pdf
