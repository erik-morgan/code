...
function layersAndText(){
    ...
    findChangeGrep({'findWhat':' : '}, {'changeTo':': '});
...
function fixFrames(){
    for (var i = 0, mfLength = mainFrames.length; i < mfLength; i++){
        ...
        if (mainFrame.paragraphs.length > 0){
            ...
            var x2 = x1 + 486;
            var y1 = (mainPage.documentOffset == 0) ? 67.5 : ((mainFrame.paragraphs[0].appliedParagraphStyle.name == 'Level 2' || mainFrame.paragraphs[0].appliedParagraphStyle.name == 'Level 2 (Tools)') ? 130.5 : 76.8);
//            var y2 = (i == mfLength - 1 && mainFrame.overflows) ? 738 : mainFrame.geometricBounds[2];
            if (i == mfLength - 1){
                var y2 = 738;
            }
            else {
                var yValues = [67.5, 76.8, 130.5, 144, 180, 216, 252, 288, 324, 360, 396, 432, 468, 504, 540, 576, 612, 648, 684, 720, 738];
                var yText = mainFrame.characters[-1].baseline;
                for (var g = 0; g < mainPage.groups.length; g++){
                    var pItem = mainPage.groups[g];
                    if (pItem.textWrapPreferences.textWrapMode !== TextWrapModes.NONE && pItem.geometricBounds[2] > yText && pItem.geometricBounds[3] > (mainPage.bounds[0] + 180)){
                        yText = pItem.geometricBounds[2];
                    }
                }
                if (yValues.join(' ').search(mainFrame.geometricBounds[2]) > -1 && mainFrame.geometricBounds[2] > yText){
                    var y2 = mainFrame.geometricBounds[2];
                }
                else {
                    for (j = 0, yLength = yValues.length; j < yLength; j++){
                        var guideOver = yValues[j];
                        var guideUnder = yValues[j + 1];
                        if (guideOver <= yText && yText <= guideUnder){
                            mainFrame.geometricBounds[2] = guideUnder;
                            var y2 = (mainFrame.characters[-1].baseline !== yText) ? yText : guideUnder;
                            break;
                        }
                    }
                }
