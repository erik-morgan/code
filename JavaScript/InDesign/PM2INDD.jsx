#target indesign
//var pmFiles = [];
var root = Folder('/Users/HD6904/Erik/PageMaker Scripting Project/Final PageMaker Folder/');
//getChildren(root);
...
function getChildren(parent){
	...
	for (var i = 0; i < children.length; i++){
		...
		else if (!(/^Aber|^BZ|^CC/.test(children[i].name)) && children[i].name.substr(-3) == 'p65'){
...
for (var i = 0; i < pmFiles.length; i++){
	...
	try {
		...
		try {
			...
			logToFile('Skipped due to damaged source file');
		}
	}
	if (doc.isValid && doc.documentPreferences.pageSize == 'Letter'){
		try {
			PM2INDD(doc);
			var savePath = '/Users/HD6904/Erik/PageMaker Scripting Project/Converted PageMaker Files/' + pmFiles[i].name.replace(/p65/i, 'indd');
