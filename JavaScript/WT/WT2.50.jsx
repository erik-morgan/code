...
function docSetup(){
	...
	for (i = 0, tfLength = masterDoc.textFrames.length; i < tfLength; i++){
		...
		if (frame.contents == 'DOCUMENTATION'){
			...
			frame.properties = {contents:sysText, geometricBounds:[769.5 - (7.2 * frame.paragraphs.length), frame.parentPage.bounds[1] + 243, 769.5, frame.parentPage.bounds[1] + 369]};
			frame.parentStory.properties = {appliedParagraphStyle:doc.paragraphStyles.item('footer'), appliedCharacterStyle:doc.characterStyles[0]};
		}
	}
...
function fixFrames(){
	for (i = 0, mfLength = mainFrames.length; i < mfLength; i++){
		...
		var frameEnd = mainFrame.geometricBounds[2];
		var frameEndMin = mainFrame.characters[-1].baseline;
		for (j = 0, yLength = yValues.length; j < yLength; j++){
			var guideOver = yValues[j];
			var guideUnder = yValues[j + 1];
			if (guideOver <= frameEndMin && frameEndMin <= guideUnder){
				mainFrame.geometricBounds[2] = guideUnder;
				var y2 = (mainFrame.characters[-1].baseline !== frameEndMin) ? frameEndMin : guideUnder;
				break;
			}
		}
...
function noteBoxes(){
	...
	for(y = 0, findLength = findNotes.length; y < findLength; y++){
