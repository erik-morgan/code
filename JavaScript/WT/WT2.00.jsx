...
function noteBoxes(){
    ...
    for(y = 0; y < findNotes.length; y++){
        var aNote = findNotes[y];
        checkFormat(aNote.words[0]);
        var pageNum = aNote.parentTextFrames[0].parentPage.name;
        if (pageNum=='1'){
            var x1 = toPoints(227.8);
            var x2 = toPoints(561.88);
        }
        else if (pageNum%2==0){
            var x1 = toPoints(209.55);
            var x2 = toPoints(543.63);
        }
        else {
            var x1 = toPoints(839.8);
            var x2 = toPoints(1173.88);
        }
        var y1 = aNote.words[0].baseline - aNote.ascent - toPoints(6.2);
        if (bulletList.length > 0){
            for (z = 0; z < bulletList.length; z++){
                if (aNote.characters[-1].index - bulletList[z].characters[0].index == -1){
                    var y2 = bulletList[z].characters[-1].baseline + bulletList[z].descent + toPoints(6.2);
                }
                else {
                    var y2 = aNote.characters[-1].baseline + aNote.descent + toPoints(6.2);
                }
            }
        }
        else {
        ...
        var noteBox = aNote.parentTextFrames[0].parentPage.rectangles.add({geometricBounds:[y1, x1, y2, x2], itemLayer:aNote.parentTextFrames[0].itemLayer.name, fillColor:'Black', fillTint:7, strokeColor:'None', textWrapPreferences:TextWrapModes.none, appliedObjectStyle:'None'});
        noteBox.sendToBack();
    }
}
...
function cleanDoc(){
    ...
    for (g = 0; g < findGreps.length; g++){
        app.findChangeGrepOptions = app.findGrepPreferences = app.changeGrepPreferences = NothingEnum.nothing;
        app.findChangeGrepOptions.includeLockedStoriesForFind = true;
        app.findGrepPreferences.findWhat = findGreps[g];
        app.changeGrepPreferences.changeTo = changeGreps[g];
        if (findGreps[g] == '[Ff]igure (\\d+)'){
    ...
    for (h = 0; h < allGroups.length; h++){
        var groupWidth = toPoints(allGroups[h].geometricBounds[3]) - toPoints(allGroups[h].geometricBounds[1]);
        var groupItems = allGroups[h].pageItems;
        for (i = 0; i < groupItems.length; i++){
            var item = groupItems[i];
            if (item.allGraphics.length == 1){
                item.sendToBack();
            }
            item.textWrapPreferences.textWrapModes.none;
        }
        allGroups[h].bringToFront();
        if (groupWidth > 234){
            allGroups[h].textWrapPreferences.textWrapMode = TextWrapModes.jumpObjectTextWrap;
        }
        else {
            allGroups[h].textWrapPreferences.textWrapMode = TextWrapModes.boundingBoxTextWrap;
        }
        allGroups[h].textWrapPreferences.textWrapOffset = '9pt';
        }
    }
}
...
function addRev(){
    ...
    app.findChangeGrepOptions.includeMasterPages = app.findChangeGrepOptions.includeLockedStoriesForFind = true;
    var footers = doc.findGrep();
    for (i = 0; i < grepFooters.length; i++){
        var footer = footers[i];
        if (footer.parent.textContainers[0].parentPage.name == masterDoc.pages[0].name){
            revFrames.push(footer.parent.textContainers[0]);
            var revText = footer.parent.paragraphs[0].contents.toLowerCase();
        }
    }
    if (revText.indexOf('rev') >= 0){
        var rev = new Number(revText.replace(/.*rev (\d+).*\r/, '$1')) + 1;
        revFrames[0].contents = revFrames[1].contents = revFrames[1].contents.replace(/rev (\d+)/, 'REV ' + rev);
        docName = docName.replace(/ r\d+ /, ' r' + rev + ' ');
    }
    else {
        revFrames[0].paragraphs[0].insertionPoints[-2].contents = revFrames[1].paragraphs[0].insertionPoints[-2].contents = ' REV 1';
        docName = docName.replace(/ /, ' r1 ');
    }
    revFrames[0].contents = revFrames[1].contents = revFrames[1].contents.replace(/\d{6}/, dateString);
    doc.save(File(defaultLocation + '/' + docName));
}
...
function toPoints(num){
    var docUnits = doc.viewPreferences.verticalMeasurementUnits;
    return UnitValue(num, 'points').as(docUnits);
}

function unlock(){
    doc.allPageItems.everyItem().locked = false;
