...
// Consider incorporating cleanText
// Consider adding labels and layers to DQConfig
// Rename DQConfig to DQSetup
#target indesign
var run, funcs, backup, d = new Date();
    ...
    w.alignChildren = ['left', 'center'];
            var checkDC = w.add('checkbox', undefined, '\u00A0DQ Config: Resets layers, colors, styles, guides, logos, and footers');
                checkDC.onClick = function (){
                    bRun.enabled = (checkDC.value || checkFF.value || checkCT.value || checkNB.value);
                }
        ...
        var pBackup = w.add('panel', undefined, 'Backup', {borderStyle:'etched'});
            ...
            pBackup.spacing = 6;
            var checkBackup = pBackup.add('checkbox', undefined, 'Backup? (User\'s Documents/WT Backups');
...
if (run){
    ...
    doc.marginPreferences.properties = {top:'54pt', left:'72pt', bottom:'54pt', right:'54pt'};
    ...
    try {
        ...
        alert('Couldn\'t find "SUGGESTED PROCEDURE"\nSince ALL procedure\'s will have a single occurence of suggested procedure, the script uses it to identify the main story');
    }
...
function labelsAndLayers(){
    ...
    for (i = items.length - 1; i >= 0; i--){
        ...
        if (item.itemLayer.name !== 'Old Table'){
            if (item.parentPage.name == masterDoc.pages[0].name){
                item.itemLayer = doc.layers.item('Master Default');
            }
            else {
...
function dqConfig(){
    ...
    for (i = 0; i < verticalGuides.length; i++){
        masterDoc.guides.add(doc.layers.item('Master Default'), {orientation:HorizontalOrVertical.vertical, location:verticalGuides[i] + 'pt'});
    }
    for (i = 0; i < horizontalGuides.length; i++){
        masterDoc.guides.add(doc.layers.item('Master Default'), {orientation:HorizontalOrVertical.horizontal, location:horizontalGuides[i] + 'pt'});
    }
    ...
    var logoLeft = masterDoc.place(logo, ['54pt', doc.marginPreferences.top], doc.layers.item('Master Default'), {horizontalScale:50, verticalScale:50});
    ...
    var pageL = masterDoc.textFrames.add({itemLayer:'Master Default', geometricBounds:['756.5pt', '54pt', '769.5pt', '130.5pt'], strokeColor:'None', contents:'Page '});
    ...
    var sysL = masterDoc.textFrames.add({itemLayer:'Master Default', geometricBounds:[uVal((769.5 - (7.2 * sysText.length)),   'pt'), '243pt', '769.5pt', '369pt'], strokeColor:'None', contents:sysText.join('')});
        sysL.parentStory.appliedParagraphStyle = 'footer';
    var revL = masterDoc.textFrames.add({itemLayer:'Master Default', geometricBounds:['755.1pt', '432pt', '769.5pt', '540pt'], fillColor:'None', strokeColor:'None', contents:revText});
        ...
        pageL.duplicate(undefined, ['1039.5pt', 0]);
        sysL.duplicate(undefined, ['612pt', 0]);
        revL.duplicate(undefined, ['252pt', 0]);
    //    MasterLines
    for (i = 0; i < masterDoc.graphicLines.length; i++){
        ...
        if (line.geometricBounds[0] == line.geometricBounds[2] && line.geometricBounds[3] - line.geometricBounds[1] == uVal(486, 'pt')){
            line.geometricBounds[0] = line.geometricBounds[2] = '738pt';
        }
    }
}
...
function uVal(num, unit){
    if (docUnits == 'INCHES_DECIMAL'){
        return unitValue(num, unit).as('in');
    }
    else {
...
function backupHandler(){
    ...
    var backupName = ('0' + Date().getMonth()).substr(-2) + ('0' + Date().getDate()).substr(-2) + Date().getYear.substr(-2) + ('0' + date.getHours().toString()).substr(-2) + ('0' + date.getMinutes().toString()).substr(-2) + ('0' + date.getSeconds().toString()).substr(-2) + ' ' + doc.name;
    if (wtBackups.exists){
        var backups = wtBackups.getFiles();
        for (i = 0; i < backups.length; i++){
            var aBackup = backups[i];
            if ((d.getTime() - aBackup.modified)/86,400,000 > 14){
                aBackup.remove();
            }
        }
    }
    ...
    var thisFile = File(doc.fullName);
    thisFile.copy(wtBackups + '/' + backupName);
}