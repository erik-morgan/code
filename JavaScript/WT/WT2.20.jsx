...
var description = '* Bypasses all locked content except for items on the \'Old Table\' layer.\r* Searches for \'suggested procedure\', and assumes its parent story as the main story.\r* FixGroups is still in development, and may behave unexpectedly.\r\rPosition the pointer over a function to see a detailed description.\r\r';
...
function myMouseHandler(ev){
    if (ev.type == 'mouseover'){
        if (ev.target == checkDC){
            ...
            d.text = '* Bypasses all locked content except for items on the \'Old Table\' layer.\r* Searches for \'suggested procedure\', and assumes its parent story as the main story.\r* FixGroups is still in development, and may behave unexpectedly.\r\rPosition the pointer over a function to see a detailed description.\r';
        }
    }
...
function labelsAndLayers(){
    ...
    for (p = 0; p < doc.pages.length; p++){
        doc.pages[p].label = (doc.pages[p].name % 2 == 0) ? 'EVEN' : 'ODD';
    }
    ...
    for (i = doc.pageItems.length - 1; i >= 0; i--){
        ...
        if (item.parentPage == null){
            ...
            if (item.itemLayer.name == 'Old Table'){
                ...
                if (item.parentPage.label == 'MASTER'){
...
function dqConfig(){
    ...
    if (app.version.substr(0, 3) >= 9.2) app.menus.item('Swatches Panel Menu').submenus.item('Sort').menuItems.item('All Swatches By Name').associatedMenuAction.invoke();
    //    STYLES
    ...
    //    LOGOS
    if (File('/Volumes/share/SERVICE/TWD/DQ Logo.ai').exists){
        var logo = File('/share/SERVICE/TWD/DQ Logo.ai');
    }
    else if (File('/Volumes/share/SERVICE/Writing Department Art Work/*Sam\'s Club Art/DQ Logo')){
        var logo = File('/share/SERVICE/Writing Department Art Work/*Sam\'s Club Art/DQ Logo');
    }
    else if (File('//n/SERVICE/TWD/DQ Logo.ai').exists){
        var logo = File('//n/SERVICE/TWD/DQ Logo.ai');
    }
    else if (File('//n/SERVICE/Writing Department Art Work/' + String.fromCharCode(61473) + 'Sam\'s Club Art/DQ Logo').exists){
        var logo = File('//n/SERVICE/Writing Department Art Work/' + String.fromCharCode(61473) + 'Sam\'s Club Art/DQ Logo');
    }
    $.writeln(logo);
